...
async function boot() {
  ...
  try {
    ...
    const scene = await startScene({ layer, bots: botsData, lite, variant, debug: params.has('debug'), visibleWith: [hero, jobs] });
...
// Without WebGL or with reduced motion the poster stays and the bot cards open from the list.
// Reduced motion is a preference, so the visitor can still start the scene with the button (or ?motion).
...
if (!layer) {
  ...
  console.info('[froggion] 3D scene is off: the system asks for reduced motion (prefers-reduced-motion). Use the button or ?motion.');
...
// Header: transparent over the first screen, solid once the page scrolls.
const header = document.querySelector('.site-header');
const menuButton = document.querySelector('.menu-btn');
if (header && !body.classList.contains('legal-page')) {
...
if (header && menuButton) {
  const setOpen = (open) => {
    ...
    header.classList.toggle('is-solid', open || window.scrollY > 24 || body.classList.contains('legal-page'));
  };
...
initChats(document.querySelector('[data-chats]'), { reducedMotion: matchMedia('(prefers-reduced-motion: reduce)').matches });
